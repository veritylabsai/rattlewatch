...
gcloud builds submit --tag gcr.io/YOUR_PROJECT_ID/rattlewatch
gcloud run deploy verity-api \
  --image gcr.io/YOUR_PROJECT_ID/rattlewatch \
...
gcloud run deploy verity-mcp \
  --image gcr.io/YOUR_PROJECT_ID/rattlewatch \
  --region us-central1 --allow-unauthenticated --memory 1Gi \
  --command "python" --args "-m rattlewatch mcp --http --host 0.0.0.0 --port 8001"
```
...
```
[![veritylabsai/rattlewatch MCP server](https://glama.ai/mcp/servers/veritylabsai/rattlewatch/badges/score.svg)](https://glama.ai/mcp/servers/veritylabsai/rattlewatch)
```
